...
def merge_parquets_optimized(folder_path: Union[str, Path], output_file: Path) -> pd.DataFrame:
    """Carrega e salva os arquivos .parquet diretamente em um arquivo de saída,
    usando menos memória."""
    folder = Path(folder_path)
    parquet_files = list(folder.glob("*.parquet"))

    if not parquet_files:
        print(f"⚠️ Aviso: Nenhuma arquivo .parquet encontrado em {folder_path}.")
        return pd.DataFrame()

    print(f"Processando um total de {len(parquet_files)} arquivos para {output_file.name}...")

    # Usa o modo 'w' (write) para o primeiro arquivo e 'a' (append) para os demais
    first_file = True
    ...
    for file in parquet_files:
        try:
            ...
            # Salva o DataFrame em modo 'append' no Parquet de saída
            # Para isso funcionar, o pandas precisa do motor 'pyarrow' ou 'fastparquet' (você já usa).
            mode = 'w' if first_file else 'a'
            df.to_parquet(output_file, index=False, engine='fastparquet', append=(not first_file))
            
            first_file = False
        except Exception as e:
            print(f"❌ Erro ao ler/escrever o arquivo {file.name}: {e}")

    print(f"✅ Concatenação concluída! Dados salvos em {output_file}.")
    
    # Retorna APENAS UMA AMOSTRA ou o DataFrame inteiro se a memória permitir
    # Ou, idealmente, você pode carregar o resultado para o processamento a seguir
    try:
        merged_df = pd.read_parquet(output_file)
        return merged_df
    except Exception as e:
        print(f"⚠️ Aviso: Não foi possível carregar o arquivo final {output_file.name} de volta para a memória. Erro: {e}")
        return pd.DataFrame()


# ==============================================================================
# EXECUÇÃO DA CONCATENAÇÃO OTIMIZADA
# ==============================================================================

# Defina um nome para o arquivo final (dataset completo)
OUTPUT_DATASET_PATH = INPUT_FOLDER.parent / "final_merged_dataset.parquet" 

print(f"\n--- Iniciando a Combinação Otimizada dos {INPUT_FOLDER.name} ---")

# Chama a nova função
df_final_dataset = merge_parquets_optimized(INPUT_FOLDER, OUTPUT_DATASET_PATH)

# O restante do seu código (verificação de duplicação, feature engineering)
# continua aqui, operando no df_final_dataset.
...
df_final_dataset = merge_parquets_optimized(INPUT_FOLDER)
